...
class Model_index extends CI_Model {
	...
	public function SanPhamBanChay(){
		$sql = "SELECT sanpham_donhang.*, donhang.*, sanpham.*, SUM(sanpham_donhang.SoLuong) AS sl FROM `sanpham_donhang`, `donhang`, `sanpham` WHERE sanpham_donhang.MaDH = donhang.MaDH AND sanpham_donhang.MaSP = sanpham.MaSP AND donhang.TinhTrangDH=3 AND MONTH(ThoiGian) = MONTH(CURDATE()) AND YEAR(ThoiGian) = YEAR(CURDATE()) GROUP BY sanpham_donhang.MaSP ORDER BY sl DESC LIMIT 10;";
		$result = $this->db->query($sql);
		return $result->result_array();
	}
