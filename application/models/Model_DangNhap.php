...
class Model_DangNhap extends CI_Model {
	...
	public function checkLogin($taikhoan,$matkhau){
		$sql = "SELECT COUNT(*) AS SL, `khachhang`.`TrangThai`FROM `taikhoan` INNER join `khachhang` on `taikhoan`.`MaTK`=`khachhang`.`MaTK` WHERE `taikhoan`.TenTK = ? AND `taikhoan`.MatKhau = ? AND `taikhoan`.ID = 1";
