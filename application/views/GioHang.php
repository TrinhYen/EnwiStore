			...
			<?php if (count($result) == 0) { ?>
				<div class="cart-content-left">
					...
					<div style="text-align: center;">
						...
						<a style="margin-top: 200px" href="<?php echo base_url(); ?>">
							<< TIẾP TỤC MUA HÀNG</a>
					</div>
				</div>
					...
					<table>
						...
						<?php foreach ($result as $key => $value): ?>
							<tr>
								...
								<td>
									<input class="number" data="<?php echo $value['MaSP']; ?>" type="number"
										value="<?php echo $value['SoLuong']; ?>" min="1">
								</td>
								<td>
									<p>
										<?php echo number_format($value['GiaTien']); ?>đ
									</p>
								</td>
								<td><a href="<?php echo base_url('gio-hang/xoa/' . $value['MaSP']); ?>"><i
				...
				<div class="cart-content-right" style="padding: 5px;">
					<table>
						...
						<?php foreach ($result as $key => $value): ?>
							...
							<tr>
								<td style="border: none;"><a style="color: black;"
										href="<?php echo base_url('san-pham/' . $value['MaSP']) ?>"><?php echo ($key + 1) . ". " . $value['TenSP']; ?></a></td>
								<td style="border: none;">
									<?php echo number_format($value['GiaTien']); ?>đ x
						...
						<tr>
							<td>Phí ship</td>
							<td>
								<?php echo number_format($result[0]['PhiShip']); ?>đ
							</td>
						</tr>
						<tr style="font-weight: bolder;">
							<td>TỔNG ĐƠN</td>
							<td>
								<?php echo number_format($sum + $result[0]['PhiShip']); ?>đ
							</td>
						</tr>
					</table>
...
<script>
	var result = <?php echo count($result); ?>;
	
	
	var base_url = window.location.origin == "http://localhost" ? window.location.origin + "/EnwiStore" : window.location.origin
	$(document).ready(function () {
		console.log(document.getElementById('numberProduct'))
		document.getElementById('numberProduct').innerHTML= result
